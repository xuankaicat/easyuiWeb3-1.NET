...
function editdata(datagridname,urlstr,columns,divname) {
    ...
    if (node) {  
        $.post(urlstr + '?type=edit&Id=' + node.id, function (msg) {
            ...
            //$('#txtRemark').val(str[3]);
            //$('#gg2').combogrid('setValue', str[1]);
            //$('#gg3').combobox('setValue', str[1]);
            //$('#ccom').combogrid('setValue', str[1]);
            //$('#ddtime').datebox('setValue', str[4]);

            //$('#ID').datebox('setValue',dateTime)     赋值
            //$('#ID').datebox('getValue')                    取值
        });
        $('#HdId').val(node.id);
        openwin(divname);
    } else {
